...
import mwclient, configparser, mwparserfromhell, argparse,re, pathlib, divcolfixer
...
def save_edit(page, utils, text):
    ...
    for template in code.filter_templates():
        if template.name.matches("nobots") or template.name.matches("Wikipedia:Exclusion compliant"):
            ...
            return
    if not divcolfixer.call_home(site, "DeprecatedFixerBot"):#config):
...
def main():
    dry_run = False
    is_div_col = True  # false means that it is the columns-list category
    pages_to_run = 100
    offset = 0
    category = "Pages using div col with deprecated parameters"
    #"Pages using Columns-list with deprecated parameters"#"Pages using div col with deprecated parameters"
    limited_run = True
